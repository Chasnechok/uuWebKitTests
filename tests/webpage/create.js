const url =
    'https://uuapp.plus4u.net/uu-webkit-maing02/362ef7f4f2644e5797e6107034c0cfe3/writerGate/home'
...
const WebpageCreate = async () => {
    ...
    await expect(page).toMatchElement(
        '.uu5-forms-input[name=name] input[name=name]',
        {
            timeout: 8000,
        },
    )
    ...
    // Alternative name
    await expect(page).toMatchElement(
        '.uu5-forms-input[name=title] input[name=title]',
    )
    ...
    // Description
    await expect(page).toMatchElement(
        '.uu5-forms-input-wrapper > .uu5-codekit-ace-editor > textarea',
    )
    const inputDescriptions = await page.$$(
        '.uu5-forms-input-wrapper > .uu5-codekit-ace-editor',
    )
    for (let i = 0; i < inputDescriptions.length; i++) {
        ...
        await expect(input).toFill(
            'textarea',
            faker.random.words(~~(Math.random() * 40)),
        )
    }

    // Save
    await expect(page).toClick('.uu-webkit-add-page-form-button-wrapper span', {
        text: 'Create',
    })
    ...
    await page.waitForResponse(
        async (response) => {
            try {
                ...
                if (!webPage || !webPage.code)
                    throw new Error(
                        'Response does not contain a "webpage" field or "code" field.',
                    )
                webPageCode = webPage.code
                return (
                    response.url().match(/\/addWebPage.*$/i) &&
                    response.status() === 200
                )
            } catch (error) {
                return false
